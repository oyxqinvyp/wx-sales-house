...
let nameMap = {
	...
	'本周': (value, item) => {
		return `${value}\n${item.name}(${item.data.text})`
	},
	'上周': (value, item) => {
		return `${item.name}(${item.data.text})`
	},
	'本月': (value, item) => {
		return `${value}\n${item.name}(${item.data.text})`
	},
...
const cfu = {
	...
	"formatter": {
		...
		"tooltipFormat1": function(item, category, index, opts) {
			let categoryShow = nameMap[item.name] ? nameMap[item.name](category, item) : category
			return `${categoryShow}: ${item.data.lineType == 2 ? '$' : ''}${item.data.value}`
		},
